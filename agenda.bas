...
/' Pendiente:
	- Al entrar, si no hay archivo, crear uno
...
Declare Function pfecha(ByVal fecha As String, ByVal hora As String) As String
' Tipos
Type evento
	activo As Integer
	fecha As String
...
entrada = UCASE(inkey())
IF entrada = "A" And posicion>0 Then posicion=posicion-1:refrescaPantalla(posicion, ev(posicion).activo, ev(posicion).fecha, ev(posicion).hora, ev(posicion).info)
IF entrada = "D" And posicion<maximo-1 Then posicion=posicion+1:refrescaPantalla(posicion, ev(posicion).activo, ev(posicion).fecha, ev(posicion).hora, ev(posicion).info)
IF entrada = "U" Then usr=seguridad:refrescaPantalla(posicion, ev(posicion).activo, ev(posicion).fecha, ev(posicion).hora, ev(posicion).info)
...
Sub muestraListado
	...
	For id As Integer = 0 To maximo-1
		muestraDatos id, ev(id).activo, ev(id).fecha, ev(id).hora, ev(id).info
	Next
	muestraInstrucciones
	muestraGraficos
End Sub
...
Sub muestraListadoHoy
	...
	For id As Integer = 0 To maximo-1
		...
		If (panio(ev(id).fecha)=Str(Year(Now)) Or panio(ev(id).fecha)="0000") _
			And (pmes(ev(id).fecha)=Str(rellenaPorLaIzquierda(Month(Now),2,"0")) Or pmes(ev(id).fecha)="00") _
			And	pdia(ev(id).fecha)=Str(rellenaPorLaIzquierda(Day(Now),2,"0")) Then
				muestraDatos id, ev(id).activo, ev(id).fecha, ev(id).hora, ev(id).info
		End If
	Next
	muestraInstrucciones
...
Sub muestraListadoEstaSemana
	...
	For id As Integer = 0 To maximo-1
		...
		If DateValue(ev(id).fecha)>=Now And DateValue(ev(id).fecha)<=Now+7 Then
				muestraDatos id, ev(id).activo, ev(id).fecha, ev(id).hora, ev(id).info
		End If
	Next
	muestraInstrucciones
...
Sub muestraListadoEsteMes
	...
	For id As Integer = 0 To maximo-1
		...
		If (panio(ev(id).fecha)=Str(Year(Now)) Or panio(ev(id).fecha)="0000") _
			And (pmes(ev(id).fecha)=Str(rellenaPorLaIzquierda(Month(Now),2,"0")) Or pmes(ev(id).fecha)="00") Then 
			muestraDatos id, ev(id).activo, ev(id).fecha, ev(id).hora, ev(id).info
		End If
	Next
	muestraInstrucciones
...
Sub muestraListadoEsteAnio
	...
	For id As Integer = 0 To maximo-1
		...
		If (panio(ev(id).fecha)=Str(Year(Now)) Or panio(ev(id).fecha)="0000") Then
			muestraDatos id, ev(id).activo, ev(id).fecha, ev(id).hora, ev(id).info
		End If
	Next
	muestraInstrucciones
...
Sub muestraListadoEntreFechas
	...
	Dim As String fechaInicio, fechaFin
	Print "Introduzca las fechas validas de inicio y fin para la busqueda."
	While isDate(fechaInicio)=0
		Input "Fecha inicio (dd/mm/yyyy): ", fechaInicio
		If isDate(fechaInicio)=0 Then Print "Fecha incorrecta. Escriba una fecha valida."
	Wend
	While isDate(fechaFin)=0
		Input "Fecha fin    (dd/mm/yyyy): ", fechaFin
		If isDate(fechaFin)=0 Then Print "Fecha incorrecta. Escriba una fecha valida."
	Wend
	For id As Integer = 0 To maximo-1
		' Comprobamos la correspondencia con esta semana (Falta incluir casos repetitivos)
		If DateValue(ev(id).fecha)>=DateValue(fechaInicio) And DateValue(ev(id).fecha)<=DateValue(fechaFin) Then
				muestraDatos id, ev(id).activo, ev(id).fecha, ev(id).hora, ev(id).info
		End If
	Next
	muestraInstrucciones
...
Sub cargaDatos
	...
	While Not Eof(1)
		...
		If activoTmp = 1 Or muestraPasivos = 1 Then 
			...
			ev(posicion).fecha=fechaTmp
			ev(posicion).hora=horaTmp
...
Sub ordenaArray
	...
	For n As Integer = Lbound(ev) To Ubound(ev)
		For i As Integer = Lbound(ev) To Ubound(ev)-2
			If Val(pfecha(ev(i).fecha,ev(i).hora)) > _ 
				Val(pfecha(ev(i+1).fecha,ev(i+1).hora)) Then Swap ev(i), ev(i+1)
		Next
	Next
End Sub
...
Function pfecha(ByVal fecha As String, ByVal hora As String) As String
